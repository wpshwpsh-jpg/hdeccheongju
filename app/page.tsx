        ...
        {selectedImage && (
          <>
            ...
            {markers.map((marker) => {
              ...
              return (
                <div key={marker.id} className={cn("absolute rounded-lg border px-1 py-0.5 shadow-md backdrop-blur-[1px] sm:rounded-2xl sm:px-3 sm:py-2", color.bg, color.text)} style={{ left: `${posX}%`, top: `${posY}%`, transform: "translate(-50%, -50%)" }}>
                  <div className={cn("flex flex-col items-center text-center", isHighRiskMarker ? "min-w-[64px] max-w-[88px] gap-0.5 sm:min-w-[104px] sm:max-w-[140px] sm:gap-1" : "min-w-[82px] max-w-[110px]:min-w-[150px] sm:max-w-[190px]")}>
                    {marker.equipmentType ? <><span className="rounded-full bg-white/80 px-2 py-0.5 text-[10px] font-bold leading-none shadow-sm sm:text-[11px]">{getEquipmentLabel(marker.equipmentType)}</span><span className="rounded-full bg-white/70 p-0.5 shadow-sm sm:p-1"><EquipmentIcon type={marker.equipmentType} className="h-4 w-4 sm:h-10 sm:w-10" /></span></> : null}
                    {isHighRiskMarker ? <div className="w-full rounded-md bg-white/65 px-1 py-0.5 shadow-sm sm:rounded-xl sm:px-2 sm:py-2"><div className="text-[9px] font-bold leading-none tracking-tight sm:text-[10px]">{marker.building || "동 미선택"}</div><div className="mt-1 text-[10px] font-semibold leading-tight sm:text-[11px]">{marker.company || "업체명 없음"}</div><div className="mt-1 break-words text-[11px] font-bold leading-tight sm:text-[13px]">{marker.note || "작업내용 없음"}</div></div> : <div className="w-full rounded-md bg-white/65 px-1 py-0.5 shadow-sm sm:rounded-xl sm:px-2 sm:py-2"><div className="text-[10px] font-semibold leading-tight sm:text-[12px]">{marker.company || "업체명 없음"}</div><div className="mt-1 break-words text-[11px] font-bold leading-tight sm:text-[13px]">{marker.note || "작업내용 없음"}</div></div>}
                  </div>
